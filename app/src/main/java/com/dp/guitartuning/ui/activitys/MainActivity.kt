...
import com.dp.guitartuning.databinding.ActivityMainBinding
import com.dp.guitartuning.ui.base.BaseActivity
import com.dp.guitartuning.ui.fragments.detail.DetailFragment
import com.dp.guitartuning.ui.fragments.home.HomeFragment
import com.dp.guitartuning.ui.fragments.settings.GeneralSettingsViewModel
import com.dp.guitartuning.ui.fragments.settings.SettingsContainerFragment
...
class MainActivity : BaseActivity<ActivityMainBinding>(ActivityMainBinding::inflate) {
    ...
    private fun applyStoredThemeEarly() {
        val prefs = applicationContext.getSharedPreferences(
            com.dp.guitartuning.common.data.preferences.PreferencesConstants.KEYSTORE_NAME,
            android.content.Context.MODE_PRIVATE
        )
        val raw = prefs.getString(
            com.dp.guitartuning.common.data.preferences.PreferencesConstants.KEY_GENERAL_THEME_MODE,
            null
        )
        val themeMode = com.dp.guitartuning.domain.model.AppThemeMode.fromStorage(raw)
        GeneralSettingsViewModel.applyTheme(themeMode)
    }

    private fun switchFragment(tag: String, creator: () -> Fragment) {
        val fm = supportFragmentManager
        val target = fm.findFragmentByTag(tag) ?: creator().also {
            fm.beginTransaction().add(R.id.nav_host_fragment, it, tag).commitNow()
        }
    ...
    private fun switchPrimaryTab(tabId: Int) {
        ...
        when (tabId) {
            R.id.tab_home -> switchFragment("home") { HomeFragment() }
            R.id.tab_msg -> switchFragment("detail") { DetailFragment() }
            R.id.tab_mine -> switchFragment("settings") { SettingsContainerFragment() }
        }
    }
